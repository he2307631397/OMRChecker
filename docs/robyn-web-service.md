...
| `markerConfig` or `recognitionConfig.markerConfig` | no | Generates `marker.png` and, unless `referenceConfig` is supplied, `reference.png` from `sourcePdfOsskey` during batch processing. The generated files are written into the selected template directory and copied into sheet workdirs. By default this only wires generated `reference.png` into `FeatureBasedAlignment`; it does not enable `CropOnMarkers` because the frontend template coordinates are in the original rendered-PDF coordinate system. |
| `referenceConfig` or `recognitionConfig.referenceConfig` | no | Generates `reference.png` or the configured `outputName` from `sourcePdfOsskey`. When supplied it overrides the reference generated from `markerConfig`. |
| `markerConfig.sourcePdfOsskey` | yes when `markerConfig` is supplied | Template PDF object key used to render the marker/reference source page. |
| `markerConfig.pdfPage` | no | 1-based PDF page to render. Defaults to `1`. |
| `markerConfig.pdfDpi` | no | PDF render DPI. Defaults to `144`. |
| `markerConfig.bbox` | yes when `markerConfig` is supplied | `[x, y, width, height]` crop box in rendered PDF pixel coordinates for the single marker template. |
| `markerConfig.outputName` | no | Marker file name. Defaults to `marker.png`. |
| `markerConfig.enableCropOnMarkers` | no | Boolean. Defaults to `false`. Set to `true` only when template field coordinates have been calibrated against the page produced by `CropOnMarkers`; otherwise OMRChecker keeps the original rendered-PDF coordinate system and uses generated `reference.png` for `FeatureBasedAlignment` only. |
