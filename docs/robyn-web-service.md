...
| `markerConfig` or `recognitionConfig.markerConfig` | no | Generates `marker.png` and, unless `referenceConfig` is supplied, `reference.png` from `sourcePdfOsskey` during batch processing. The generated files are written into the selected template directory and copied into sheet workdirs. |
